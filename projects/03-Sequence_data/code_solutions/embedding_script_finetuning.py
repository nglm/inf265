...
def pipeline(
    ...
    print(
        "use_unk_limit %s  |  use_tanh  %s" %(str(use_unk_limit), str(use_tanh) )
    )
    ...
    # ----------- Embedding analysis -----------
    sim, embs = similarity_matrix(vocab, model_cbow, use_tanh=use_tanh)
    words = [
...
list_context_size = [2]
list_embedding_dim = [12, 16]
list_occ_max = [np.inf]
list_use_weight = [True]
list_use_tanh = [True, False]
list_black_list = [["<unk>"]]
    ...
    for embedding_dim in list_embedding_dim:
        for occ_max in list_occ_max:
            for use_weight in list_use_weight:
                for use_tanh in list_use_tanh:
                    for black_list in list_black_list:
                        pipeline(
                          context_size, embedding_dim, 
                          occ_max=occ_max, use_weight=use_weight, use_tanh=use_tanh,
